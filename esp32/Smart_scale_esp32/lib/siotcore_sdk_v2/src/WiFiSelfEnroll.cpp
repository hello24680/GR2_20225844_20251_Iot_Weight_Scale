...
void WiFiSelfEnroll::_APISave()  {
    ...
    server.send(200,"text/plain", "WiFi saved! Rebooting now...");
    
    /// Auto-reboot after saving WiFi credentials
    #ifdef _DEBUG_             
    Serial.println("WiFi credentials saved. Rebooting...");
    #endif
    delay(3000); // Wait 3 seconds for response to be sent
    ESP.restart();
}
...
void WiFiSelfEnroll::SetupStation(const char * adhoc_ssid, const char * adhoc_password) {
    ...
    /// Handle all other requests (captive portal detection, favicon, etc.)
    server.onNotFound([]() {
        // Redirect all unknown requests to home page for captive portal
